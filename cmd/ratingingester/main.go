...
func main() {
	...
	const fileNmae = "ratingsdata.json"
	fmt.Println("Reading ratings from file: " + fileNmae)

	ratingEvents, err := readRatingEvents(fileNmae)
	if err != nil {
		panic(err)
...
func produceRatingEvents(topic string, producer *kafka.Producer, events []model.RatingEvent) error {
	for _, event := range events {
		...
		if err := producer.Produce(&kafka.Message{TopicPartition: kafka.TopicPartition{
