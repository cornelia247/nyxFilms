...
type ratingRepository interface {
	Get(ctx context.Context, recordID model.RecordID, ratingType model.RecordType) ([]model.Rating, error)

	Put(ctx context.Context, recordID model.RecordID, ratingType model.RecordType, rating *model.Rating) error
}
...
// GetAggregatedRating returns the aggregated rating for a record or ErrNotFound if thete are no ratings for it.
...
// StartIngestion starts the ingestion of ratings events.
