...
// New initialises a new rating gRPC handler.
...
func (h *Handler) PutRating(ctx context.Context, req *gen.PutRatingRequest) (*gen.PutRatingResponse,error) {
	if req == nil || req.RecordId == "" || req.UserId == "" {
		return nil, status.Error(codes.InvalidArgument, "nil req or empty user id or empty record_id")
	}
	if err := h.ctrl.PutRating(ctx, model.RecordID(req.RecordId),model.RecordType(req.RecordType), &model.Rating{UserID: model.UserID(req.RecordId),Value: model.RatingValue(req.RatingValue)})
	err != nil {
		return nil, err
	}
